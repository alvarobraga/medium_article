...
Filter
		;save the address of the variables Data->copy_of_samples,
		;Data->rms_squared and Data->gain into the stack for later use
		PUSH	{R1}
		PUSH	{R0}
		PUSH    {R2}
...
;*** Apply filter
LOOP
		LDR	       R1,[R0]           ;R0 holds the memory address of the array samples[1000], 
		                                 ;passed as argument in the C program
	...
	BEQ     LOOP_ONE	                 ;If loop counter is equal to 1, jump to tag LOOP_ONE. Continue otherwise		
		...
		ADDNE	       R0,R0,#4          ;Since data is stored as Little Endian in ARM processors, 
		                                 ;adding 4 points to the next element in the array samples
...
Coefficients
		...
		DCD     0x3A03126F      ;1/2000. = 0.0005 -- sampling period
