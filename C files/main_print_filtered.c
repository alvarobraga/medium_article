...
typedef struct
{
    ...
    float filtered_values[SAMPLING_SIZE]; // Variable that will receive filtered values
} Data_t;
...
extern void Filter(uint32_t *sampling_vector, float *rms_squared, uint32_t *gain, float *filtered_values);
...
static void dma_callback(void)
{
    ...
    Filter(Data->copy_of_samples, &Data->rms_squared, &Data->gain, Data->filtered_values);

    int n;
    for (n = 0; n < SAMPLING_SIZE; n++)
        printf("%.4f,\n\r", Data->filtered_values[n]);
